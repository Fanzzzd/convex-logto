...
- **Session-mode invariants** (each is load-bearing; read the surrounding comment before changing one):
  ...
  - When a refresh outcome is unknown, keep the claim and let it age into `claim-expired`. Only release it when the failure proves Logto never processed the request.
