...
- `e2e/` — live checks against a **real** Logto and Convex deployment. Deliberately outside the pnpm workspace and outside CI (needs credentials, a browser, a running app). `provision.mjs` is idempotent find-or-create-and-repair for the Logto objects; `session-flow.mjs` drives Chrome through the whole session lifecycle. Reach for it when a question is empirical — real token lifetimes, whether a grant rotates its refresh token, what the SSO cookie does on a second sign-in — because unit tests answer none of those.
