...
// THE RULE: webhooks may write only Logto-owned fields, meaning email, name, and
// the `status` lifecycle (active/suspended). They must never touch `role`, which
// is app-owned; otherwise a Logto profile edit (`User.Data.Updated`) would reset
// everyone's role to the default.
//
// Mirror only the fields the event carries: present → set it (a `null` clears
// it), absent → leave it. So a profile edit can clear an email, while a
...
// The webhook only SYNCS rows that already exist. It never creates them. An
// authenticated mutation (`completeOnboarding` in users.ts) creates rows, because
...
// Logto sends one, no-ops, since there's no row to sync yet.)
