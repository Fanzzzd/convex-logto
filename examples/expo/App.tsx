  ...
  return (
    <View style={styles.stack}>
      <Button
        ...
        // rejection. A sign-out that fails leaves the user signed in, which is
...
export default function App() {
  return (
    ...
    // Convex's <AuthLoading> covers the token handshake after it.
    <ConvexLogtoProvider
      client={convex}
      configQuery={api.logto.config}
      redirectUri="io.logto://callback"
      fallback={<Spinner label="Loading config…" />}
      // `@logto/rn` rejects instead of storing the error. Without this, the two
      // failures an app sees in practice are invisible: the user dismissing the
      // system browser, and a sign-out that could not reach Logto.
