...
//
// Unit tests cannot reach what this covers: Logto's real token lifetimes,
// whether a grant rotates its refresh token, what the SSO cookie does on a
// second sign-in, and how the component behaves when the wall clock advances
// instead of a fake timer. Every session-mode defect that took longest to find
// was in that gap.
//
...
// Install Chrome yourself; `playwright-core` drives it rather than downloading
// its own.
//
// Every step is a required assertion. The script skips nothing. A check that
// quietly did not run is worse than one that failed, because it reports as a
// pass.
...
// them there. That role is the one `examples/vite-react-session`'s `adminOnly`
// query requires.
...
/**
 * The library's own state defines "authenticated", not the app's copy: the
 * stored ID token plus a rendered `sub`. Reading the app's text alone would pass
 * on a page that kept stale UI.
 */
 ...
 * question about *Logto's* storage. It answers "no session token" no matter
...
/**
 * Sign out, and wait for the logout to reach Logto and come back.
 *
 ...
 * redirect has started. Anything that navigates into that window cancels the
 * request to Logto, and the OP session survives a sign-out that every local
 * assertion says succeeded. Waiting for the end-session request itself is the
 ...
 *
 * A federated sign-out is a chain: app, then Logto's end-session, then back.
 * The last hop can still be committing when the signed-out UI is already on
 * screen. A navigation issued into that window aborts. Retrying is not papering
 * over a product bug. The chain is asynchronous, and the assertion that follows
 ...
 * rejection decide the whole question. A sign-in click navigates, which
 ...
 *
 * So this reads both conditions in one evaluate, treats a failed read as "ask
 * again", and lets only the deadline answer "neither".
 */
 ...
 *
 * This runs right after a sign-in click, so the browser may be sitting on an
 * authorization request or a callback, such as `?code=…&state=…` or a fragment
 ...
 * log. The names alone still answer the question. Seeing `code` and `state`
 * present is what tells you the browser reached the callback.
 */
/**
...
function safeUrl(raw) {
  ...
  // `about:blank`, the very page this diagnostic exists for, has origin
...
/**
 * Where the browser ended up, for the third outcome.
 *
 ...
 * concerned. That is the only way to test revoking one from the other.
 */
 ...
 *
 * The name, not the URL. Every session-mode call goes to one HTTP endpoint, so
...
page.on("request", (request) => {
  ...
  try {
    ...
    // evidence, and the record must not lose it.
  }
...
try {
  ...
  // 2. Zero-RTT restore. Timing proves nothing, because a fast round trip looks
  ...
  //    is a rotated token the browser never persisted, and that only shows up
  //    on the *next* refresh, which is why this runs the cycle a second time.
  ...
  for (const round of [1, 2]) {
    ...
    assert(
      next !== previous,
      `round ${round}: the session token did not rotate. Either the component ` +
  ...
  //    is checkable offline. Whether Logto puts `organizations` and
  //    `organization_roles` in the *ID* token for the configured scopes is a
  //    property of the deployment, and the whole helper family depends on it
  ...
  //    refresh grant, so a cache that never hits would multiply this
  //    deployment's traffic to Logto by however often the app asks, and nothing
  //    would report it. `forceRefresh` is the deliberate way past it, and there
  //    is no way to prove it bypasses a cache without first proving the cache
  //    exists.
  ...
  // Never requested, so the exchange must never return it. `exposeAccessTokens`
  // is off in this app, and a token string leaking without it is the failure
  // that would be silent everywhere else.
  assert(
  ...
  // both makes Playwright refuse rather than pick. That is the right call, and
  ...
  // 8. Sign in again. Sign-out is federated by default, so it ends Logto's SSO
  //    session as well as the local one, and this must *not* be silent. The
  //    credential prompt is the only evidence that the RP-initiated logout
  //    reached Logto. Clearing local storage looks identical from the app
  //    either way, and a surviving OP session would sign the next visitor
  //    straight back in.
  await page.getByRole("button", { name: /sign in/i }).click();
  ...
  step("re-sign-in", "prompted, so the federated sign-out reached Logto");
  ...
  //    most moving pieces. A marker commits before the rows drain, and a row
  //    that is logically revoked has to stop being an authority, and stop
  ...
  try {
    ...
    // sessions, because every earlier run left some behind, so "the other one"
    // is not a thing the list can be asked for. Naming the target also means
    // the revoke below aims at a session this run created, never at a stranger.
    ...
    assert(
      ...
      `${label} is this device, so the rename landed on the wrong session`,
    );
  ...
  // 10. Sign out everywhere. The one guarantee a single browser cannot check is
  //     that a *different* device, which never sees the click, loses its
  //     session too, live and without asking for it.
  const third = await newDevice();
  try {
    await signInOnFreshDevice(third.page);
    await signOutAndWaitForLogto(page, /^sign out everywhere$/i);
    // Same settling as step 7, for the same reason. The sign-out chain's last
    // hop can still be committing, and a storage read into that window runs in
    // a document that is being replaced.
    await reopenApp(page);
    await waitForSignedOut(page);
    await waitForSignedOut(third.page);
    // Both devices. Sign-out-everywhere is a different code path from
    // sign-out, and "cleared UI, live credentials in storage" is as much a bug
    // on the device that clicked as on the one that did not.
    assert(
