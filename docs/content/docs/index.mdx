...
- **One source of truth across environments.** The frontend can pull its Logto
  config from the backend, so you configure Logto in one place per environment,
...
- **[Session mode](/docs/session-mode)**: a Convex component holds the Logto
  ...
  browser. The browser holds nothing long-lived, revocation lands live, and
  there is no Logto SDK in the bundle.
...
Logto-side requirement is that the OIDC signing key be **RSA/RS256** (Convex
