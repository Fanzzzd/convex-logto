...
`{ sessions, truncated }` — at most 16 of the caller's own sessions, newest
first, each `{ sessionId, current, createdAt, lastRefreshedAt, label?, client?,
deviceBound }`. `renameSession({ sessionToken, targetSessionId, label? })` and
