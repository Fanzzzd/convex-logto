...
`revokeSession({ sessionToken, targetSessionId })` return a boolean. All three
...
- `renameSession(sessionId, label)` — name one of the caller's own sessions;
  `undefined` clears it. Returns `false` for an id that is not the caller's.
- `revokeSession(sessionId)` — sign that device out. Revoking the **current**
  session does not clear this browser's credentials; use `signOut()` for that.
  Like `signOutEverywhere`, a missing re-export fails with the same explicit fix.
