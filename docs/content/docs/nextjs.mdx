---
title: Next.js
description: Mount the provider behind a "use client" boundary in the Next.js App Router. Session mode adds a cookie transport and server rendering with a real identity.
---
...
No Logto config reaches the bundle; session mode keeps every value on the
Convex deployment.
...
- The browser attaches a cookie to **every** same-origin request, so the ID
  ...
  That is the custody trade-off, and it is why this is opt-in; see
...
[Bridge mode](/docs/bridge-mode) keeps the same client boundary and the same
...
The runnable [`examples/nextjs`](https://github.com/Fanzzzd/convex-logto/tree/main/examples/nextjs)
