...
— the cookie transport mounted at `app/api/logto/[route]/route.ts`, the identity
rendered on the server with `preloadQuery` in `app/page.tsx`, and the
document-only refresh in `proxy.ts`, all wired together.
