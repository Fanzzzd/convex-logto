...
- [Device binding](/docs/session-mode#device-binding-optional) keeps the token in
  ...
  refresh and revocation operation. Prefer it for a static SPA when preventing a
  copied token from working on another device is the goal. It does not stop script
