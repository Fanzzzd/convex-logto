...
behind a custom domain or a reverse proxy, say — a missing `openid` scope, and a
proxy or WAF interstitial returned where the token endpoint's JSON was expected.
Any of these would otherwise destroy every session in the deployment, one refresh
at a time.

When Logto answered but the response could not be used, the component persists
any refresh token Logto rotated before failing. Presenting a superseded refresh
token again would trip Logto's reuse detection and destroy the whole grant —
including the sibling sessions that share it.
