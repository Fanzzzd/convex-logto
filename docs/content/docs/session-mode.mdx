...
A refresh can fail for two very different reasons, and the component keeps them
apart. If Logto rejects the *grant* — the refresh token was revoked, reused, or
the SSO session ended — that is terminal: the row is deleted and the user signs
in again. If the failure describes your **deployment** instead, the session is
kept and the error is transient, so fixing the environment variable is all the
recovery anyone needs.
...
One case stays conservative: a response the component cannot parse as a token
response at all — a proxy or WAF interstitial where JSON was expected, or a body
too large to read. Whether Logto processed the grant is then genuinely unknown,
so the refresh claim is left to expire and that session requires a fresh sign-in.
Guessing there is what risks spending a rotated token twice.
