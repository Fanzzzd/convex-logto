...
killed server-side, by a sign-out in another tab, token-theft detection, or a
webhook suspension, Convex pushes `false` and the app drops to signed-out
**live**, not at token expiry.
...
SSO session ended, that is terminal. The row is deleted and the user signs in
again.
...
what actually went wrong. A failure that never reached the deployment at all is
retried once, since nothing was consumed.)
...
merge), per browser (Web Locks), per session (a server-side claim). A concurrent
replay of the same Logto refresh token at its rotation boundary would destroy
the grant. Server claims are ownership-fenced. If a claim exceeds its
...
| `onAuthError` | none | Sign-in initiation and callback failures, plus opted-in device-key storage failures. Initiation failures are reported before `signIn()` rejects, so `void signIn()` remains observable. |
