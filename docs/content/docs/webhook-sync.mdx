...
Create the row from an **authenticated mutation** instead; it runs as the
signed-in user, so it always fires. Let the webhook keep it in sync afterward:
...
In [session mode](/docs/session-mode), also pass the component:
...
That adds two things. **Exactly-once handling.** The route deduplicates
...
handlers mapped (`logtoSync({})`); the [session example](https://github.com/Fanzzzd/convex-logto/tree/main/examples/vite-react-session)
does exactly that. [Bridge mode](/docs/bridge-mode) has no sessions to revoke,
...
Then copy the **Signing key**. It is a secret, like the app secret, and lives
on the deployment:
...
The route verifies the signature with Web Crypto (HMAC-SHA256) inside the Convex
runtime.
