...
<Callout>
Backend setup (Logto app + RSA key, Convex env, `auth.config.ts`) is the same for
every framework; see [Quick start](/docs/quick-start) steps 1 to 3.
</Callout>
...
sign-in. Add `navigate` once you have a router, as the
[TanStack Router](/docs/tanstack-router) page does.
...
Without a router, render a callback view when the path matches:
...
static-hosting path. The session token there is one-time and rotates on every
use, and the server stores only its hash. Add the cookie transport when you have
a server on the same site, not before.
</Callout>
