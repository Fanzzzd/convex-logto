...
**Rotate the signing key to RSA first.** Convex only accepts ID tokens signed with
**RS256** (or EdDSA). Logto signs with **ES384** by default, which Convex rejects
without an error. Sign-in looks like it works, but `ctx.auth.getUserIdentity()`
...
The same as the web setup; the backend is framework-agnostic:
...
| Loading | SSR-safe, renders children with no fallback | `fallback` covers the one-time config fetch |

Everything else is shared: `logtoAuthConfig`, `logtoConfigQuery`, the webhook
[user-sync](/docs/webhook-sync), and reading `ctx.auth.getUserIdentity()`.
...
token is a cold-start choice. While the token is fresh and its paired component
Session marker is still present, the app authenticates on launch without a
refresh. The provider clears an orphan ID token. Unlike AsyncStorage, the
...
| Software device binding | optional on web | none, on purpose; SecureStore uses the OS keystore |
...
reports the failure through `onAuthError`; the provider likewise reports a
sign-in action or system-browser initiation failure before `signIn()` rejects.
It never falls back to AsyncStorage or an unbound plaintext credential.

## Expo SDK and `@logto/rn` versions

This repository tests `@logto/rn@1.2` with Expo SDK 56. Keep the Logto adapter
and Expo modules within their declared peer ranges when upgrading; the versions
in this guide need no SDK 53 downgrade.
