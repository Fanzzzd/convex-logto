...
**You need organization *permissions*, and organization roles are not enough.** Logto issues
...
- **Bridge and session mode present the same ID token to Convex**, so identity,
  webhook sync, and environments are identical downstream, and moving between
  them is an import change.

## Status

Pre-1.0, so breaking changes land in minor versions until 1.0.
