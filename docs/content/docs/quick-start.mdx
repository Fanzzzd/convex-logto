...
The app secret is the one Logto value that is a secret. It stays on the
deployment and never reaches the browser.
...
and `logtoSessionApi`. This is a compatibility escape hatch. Terminate TLS
...
- [Session mode](/docs/session-mode) covers what you now have: the device
  ...
  for apps with a same-site server, and how a failed refresh is classified.
