...
The **Vite + React** happy path, top to bottom. Copy it as is and your Convex
functions will see a signed-in user. On **TanStack Router / Start, Next.js, or Expo**?
Steps 1 to 3 are the same; the provider and callback differ; see
...
a query, or a fragment. The library requires HTTPS in production; loopback HTTP
works for local development. If an existing self-hosted deployment can only be
reached over non-loopback HTTP, add `allowInsecureHttp: true` to
`logtoAuthConfig`, the static provider `config` (or `logtoConfigQuery`), and
`logtoSessionApi` where used. This is a compatibility escape hatch, not a
recommended setup.
...
`fallback` prop while it loads. Static `config` is the default because it
removes a config round trip from first paint, so sign-in is interactive on the
first render.
</Callout>
...
the route just needs to render. In a plain Vite app with no router, render a
callback view when the path matches:
...
fails, for example when Logto is unreachable or browser storage is blocked, the
...
your own (`location.href = ...`). `signOut()` resolves once it has told the
browser to leave for Logto, not once the browser has arrived, and a second
full-page navigation supersedes the request that ends the Logto session.
