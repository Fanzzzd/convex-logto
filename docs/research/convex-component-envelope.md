...
- `src/component/convex.config.ts`：`defineComponent("workOSAuthKit")`，并嵌套 `component.use(workpool, { name: "eventWorkpool" })` 与 `component.use(workflow, { name: "backfillWorkflow" })`，是 **component 组合 component** 的先例。
