...
4. **TTL 默认 14 天，per-app 可配 1 到 180 天（Console 与 Management API 均可）。** 另有两条独立上限：Grant TTL 硬编码 180 天（refresh 处理器不续期 grant，grant 过期后 refresh 直接失败）；不带 `offline_access` 签发的 refresh token 绑定用户 session（session 默认 14 天）。
...
- **Logto 对 confidential client 也会轮换**，时机是 token 已过 ≥70% TTL 时。契约不能假设 refresh token 恒定：每次 token 响应都可能带新 `refresh_token`，**必须先原子持久化新 token 再认为 refresh 完成**；持久化失败后用旧 token 重试 = 复用 consumed token = 整族吊销、用户被迫重登。
